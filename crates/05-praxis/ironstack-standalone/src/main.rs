...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "💎 Identified {} industrial presets.",
        factory_presets.count()
    );
    ...
    for i in 0..factory_presets.count() {
        if let Some(preset) = factory_presets.get(i) {
            ...
            if (ui.drive_val - preset.snapshot.get(0)).abs() > 0.001 {
                return Err(anyhow::anyhow!(
                    "❌ UI Synchronization Failure in preset: {}",
                    preset.name
                ));
            }
        }
