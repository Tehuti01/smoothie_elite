 ...
 * ╭──────────────────────────────────────────────────────────────────────────╮
 * │ FILE ID: SER-0x8f1ab7c5 | REVISION: 2026.04.20                           │
 * │ PATH: smoothie_elite/crates/02-resonance/smoothie-physics/src/wdf/tubes/6v6.rs                                                         │
 * ├──────────────────────────────────────────────────────────────────────────┤
...
/// Technical implementation of the SixV6 structure.
pub struct SixV6<
    'a,
    PlatePort: WdfNode,
    ScreenPort: WdfNode,
...
impl<'a, PlatePort: WdfNode, ScreenPort: WdfNode, CathodePort: WdfNode, GridPort: WdfNode>
    SixV6<'a, PlatePort, ScreenPort, CathodePort, GridPort>
{
    ...
    pub fn process(&mut self) {
        ...
        for _ in 0..4 {
            ...
            let i_s = if v_sk > self.beam_voltage {
                0.0008 * (v_sk - self.beam_voltage) * (v_sk - self.beam_voltage)
            } else {
                0.0
            };
