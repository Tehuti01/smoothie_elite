 ...
 * ╭──────────────────────────────────────────────────────────────────────────╮
 * │ FILE ID: SER-0x4f43cdab | REVISION: 2026.04.20                           │
 * │ PATH: smoothie_elite/crates/02-resonance/smoothie-physics/src/wdf/tubes/6l6.rs                                                         │
 * ├──────────────────────────────────────────────────────────────────────────┤
...
/// Technical implementation of the SixL6 structure.
pub struct SixL6<
    'a,
    PlatePort: WdfNode,
    ScreenPort: WdfNode,
...
impl<'a, PlatePort: WdfNode, ScreenPort: WdfNode, CathodePort: WdfNode, GridPort: WdfNode>
    SixL6<'a, PlatePort, ScreenPort, CathodePort, GridPort>
{
    ...
    pub fn process(&mut self) {
        ...
        for _ in 0..4 {
            ...
            let i_s = if v_sk > self.screen_cutoff {
                0.0009 * (v_sk - self.screen_cutoff) * (v_sk - self.screen_cutoff)
            } else {
                0.0
            };
